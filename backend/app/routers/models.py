...
from ..models import User, Version, File, ModelBasic, ModelTags, ModelMetrics
...
def list_models_v2(
    ...
    return [model_basic_to_model_info(m, db) for m in q.limit(100).all()]
...
def gather_model_metrics(model_id: int, db: Session) -> ModelInfoMetric:
    ...
    if not m:
        return ModelInfoMetric(
            parameters=None, quantatization=None, gpu_memory_gb=None, storage_gb=None
        )
    return ModelInfoMetric(
