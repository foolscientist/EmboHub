...
function renderSelectedFiles() {
  ...
  listEl.innerHTML = pendingFiles
    ...
    .join("");
  Array.from(listEl.querySelectorAll("button[data-remove]")).forEach((btn) => {
    btn.onclick = () => {
...
function renderNav() {
  ...
  if (h === "#models" || h.startsWith("#model-")) target = "navModels";
  else if (h === "#new") target = "navNew";
  else if (h === "#me") target = "navMe";
  else if (h === "#stats") target = "navStats";
  else if (h === "#admin") target = "navAdmin";
  else if (h === "#traffic") target = "navTraffic";
  if (target) {
...
function makeBrowseModelItem(m) {
...
async function viewBrowse() {
  ...
  document.getElementById("search").onclick = () => {
    ...
    const nt = document.getElementById("tags").value;
    const ns = document.getElementById("sort").value;
...
async function viewCreate() {
  ...
  const formatOption = ({ id, name }, lastId) => {
  ...
  const uploadCard = `<div class="row">
  ...
  if (btn) {
    btn.onclick = async () => {
      ...
      try {
        ...
        const fe2 = document.getElementById("fileCreate");
        if (fe2) fe2.value = "";
...
async function viewMe() {
  ...
  mount(
    `<div class="container"><div class="row"><div class="card"><div class="row"><strong>我的模型</strong><span>${mine.length}</span></div></div><div class="card"><div class="row"><strong>文件数</strong><span>${files}</span></div></div><div class="card"><div class="row"><strong>占用存储</strong><span>${toMB} MB</span></div></div></div><div class="grid">${mine.map((m) => `<div class='list-item'><div class='row' style='align-items:center'><strong style='flex:1'>${m.name}</strong><span class='muted' style='width:90px;text-align:right'>下载 ${m.download_count || 0}</span></div><div class='muted'>${m.description || ""}</div><div class='row'><button class='btn' onclick="location.hash='#model-${m.id}'">详情</button></div></div>`).join("") || `<div class='card'><div class='row'><span class='muted'>暂无模型</span><button class='btn primary' onclick="location.hash='#new'">去创建模型</button></div></div>`}</div></div>`,
  );
}
...
function enhanceCreateUI() {
  ...
  btn.onclick = async () => {
    ...
    const err = document.getElementById("err");
    const bar = document.getElementById("createBar");
    const fileEl = document.getElementById("fileCreate");
  ...
  if (fileEl) {
    ...
    fileEl.addEventListener("change", () => {
      ...
      const names = pendingFiles.map(
        (f) => `${f.name} · ${Math.round((f.size / 1024 / 1024) * 10) / 10}MB`,
      );
...
async function viewTraffic() {
  ...
  await load();
  if (window.__trafTimer) clearInterval(window.__trafTimer);
  window.__trafTimer = setInterval(load, 1000);
}
